...
echo
echo "************************************************************************"
echo "*               Testing pass: re-elaboration                           *"
echo "************************************************************************"
echo
...
EXT=${ARCH}_${OS}
if [ ! x$ACT_TEST_INSTALL = x ] || [ ! -f ../test_reelab.$EXT ]; then
  ACTTOOL=$ACT_HOME/bin/test_reelab
  echo "testing installation"
echo
else
  ACTTOOL=../test_reelab.$EXT
fi
...
do
	...
	$ACTTOOL $i  > runs/$i.t.stdout 2> runs/$i.tmp.stderr
