...
for i in $list
do
	$ACTTOOL -cnf=decomp.conf -Wno_local_driver:on $i 'foo<>' > runs/$i.stdout 2> runs/$i.tmp.stderr
	sort runs/$i.tmp.stderr > runs/$i.stderr
	rm runs/$i.tmp.stderr
	$ACTTOOL -cnf=decomp.conf -Wno_local_driver:on -v $i 'foo<>' > runs/$i.stdoutv 2> runs/$i.tmp.stderrv
