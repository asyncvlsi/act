...
#include "act/miniscm/lispargs.h"
...
LispCopyObj (LispObj *l)
{
  ...
  case S_FLOAT:
    ...
    break;
  case S_STRING:
    LSTR(s) = Strdup (LSTR(l));
    break;
  case S_BOOL:
    LBOOL(s) = LBOOL(l);
    break;
  default:
    fatal_error ("Fatal error in copy-object!\n");
    break;
  }
  return s;
}
...
void LispGCRemoveSexp (Sexp *s)
{
  extern LispObj *LispMainFrameObj;
  LispObj *l;
  
  if (LLIST(CAR(LLIST(LispMainFrameObj))) != s) {
    warning ("Fatal internal error. Proceed at your own risk!\n");
    return;
  }
