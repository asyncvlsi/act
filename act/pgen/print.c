...
void print_header_prolog (pp_t *pp)
{
  ...
  pp_printf (pp, "#include \"%s%s_parse.h\"", path_prefix, prefix);
  pp_forced (pp, 0);
  sprintf (buf, "%s_walk.extra.h", prefix);
  if ((fp = fopen (buf, "r"))) {
    pp_printf (pp, "#include \"%s%s\"", path_prefix, buf); pp_forced (pp, 0);
    fclose (fp);
  }
...
void print_walker_prolog (pp_t *pp)
{
  ...
  pp_printf (pp, "#include \"%s%s_parse.h\"", path_prefix, prefix); pp_nl;
  pp_printf (pp, "#include \"%s%s_parse_int.h\"", path_prefix, prefix); pp_nl;
  pp_printf (pp, "#include \"%s%s_walk_%s.h\"", path_prefix, prefix, WNAME); pp_nl;
